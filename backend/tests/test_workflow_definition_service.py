...
class TestGetDefinition:
    ...
    async def test_get_nonexistent_returns_none(self, workflow_definition_service):
        """존재하지 않는 정의 조회 시 None을 반환한다."""
        result = await workflow_definition_service.get_definition("nonexistent-id")
...
class TestUpdateDefinition:
    ...
    async def test_update_nonexistent_returns_none(self, workflow_definition_service):
        """존재하지 않는 정의 수정 시 None을 반환한다."""
...
class TestDeleteDefinition:
    ...
    async def test_delete_nonexistent_returns_false(self, workflow_definition_service):
        """존재하지 않는 정의 삭제 시 False를 반환한다."""
        deleted = await workflow_definition_service.delete_definition("nonexistent-id")
        assert deleted is False

    async def test_delete_builtin_raises_value_error(
        self, workflow_definition_service, db
    ):
        """is_builtin=True인 정의 삭제 시 ValueError가 발생한다."""
        ...
        with pytest.raises(ValueError, match="시스템 워크플로우는 삭제할 수 없습니다"):
...
class TestSetDefault:
    ...
    async def test_set_default_nonexistent_returns_none(
        self, workflow_definition_service
    ):
        """존재하지 않는 정의를 기본으로 설정하면 None을 반환한다."""
        result = await workflow_definition_service.set_default("nonexistent-id")
        assert result is None
...
class TestExportDefinition:
    ...
    async def test_export_nonexistent_returns_none(self, workflow_definition_service):
        """존재하지 않는 정의 export 시 None을 반환한다."""
        result = await workflow_definition_service.export_definition("nonexistent-id")
        assert result is None
...
class TestUpdateBuiltinProtection:
    ...
    async def test_builtin_name_change_raises(self, workflow_definition_service, db):
        """is_builtin=True인 정의의 이름을 변경하면 ValueError가 발생한다."""
        from datetime import datetime, timezone
        ...
        with pytest.raises(ValueError, match="시스템 워크플로우의 이름은 변경할 수 없습니다"):
