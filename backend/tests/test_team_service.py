...
class TestGetTeam:
    ...
    async def test_get_nonexistent_team_returns_none(self, team_service):
        """존재하지 않는 팀 조회 시 None을 반환한다."""
        team = await team_service.get_team("nonexistent-id")
        assert team is None
...
class TestUpdateTeam:
    ...
    async def test_update_nonexistent_team_returns_none(self, team_service):
        """존재하지 않는 팀 수정 시 None을 반환한다."""
        result = await team_service.update_team("nonexistent-id", name="x")
...
class TestDeleteTeam:
    ...
    async def test_delete_nonexistent_team_returns_false(self, team_service):
        """존재하지 않는 팀 삭제 시 False를 반환한다."""
        deleted = await team_service.delete_team("nonexistent-id")
...
class TestAddMember:
    ...
    async def test_add_member_to_nonexistent_team_raises(self, team_service):
        """존재하지 않는 팀에 멤버 추가 시 ValueError가 발생한다."""
        with pytest.raises(ValueError, match="팀을 찾을 수 없습니다"):
...
class TestRemoveMember:
    ...
    async def test_remove_nonexistent_member_returns_false(self, team_service):
        """존재하지 않는 멤버 제거 시 False를 반환한다."""
        team = await team_service.create_team("No Remove Team")

        removed = await team_service.remove_member(team.id, 999999)
...
class TestSetLead:
    ...
    async def test_set_lead_nonexistent_member_raises(self, team_service):
        """존재하지 않는 멤버를 리드로 설정 시 ValueError가 발생한다."""
        team = await team_service.create_team("No Lead Team")

        with pytest.raises(ValueError, match="해당 멤버를 찾을 수 없습니다"):
