...
class WorkflowDefinitionService:
    ...
    async def get_definition(self, def_id: str) -> WorkflowDefinitionInfo | None:
        async with self._db.session() as session:
            ...
            return self._entity_to_info(entity) if entity else None
    ...
    async def export_definition(self, def_id: str) -> dict | None:
        ...
        if not info:
            return None
        return {"version": 1, "definition": info.model_dump()}

    async def import_definition(
        self, data: WorkflowDefinitionInfo
    ) -> WorkflowDefinitionInfo:
        """export 데이터에서 새 정의를 import. 이름 중복 시 카운터 추가."""
        base_name = data.name
        name = base_name
        counter = 2
        async with self._db.session() as session:
