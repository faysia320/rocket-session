      ...
      {parts.map((part, i) => {
        if (part.startsWith("```") && part.endsWith("```")) {
          ...
          return (
            ...
            >
              {lang ? (
                <div className="font-mono text-2xs text-muted-foreground/70 mb-1.5 uppercase tracking-wider">
                  {lang}
                </div>
              ) : null}
              <code>{code}</code>
            ...
            {inlineParts.map((ip, j) => {
              if (ip.startsWith("`") && ip.endsWith("`")) {
                return (
                  <code
                    key={j}
                    className="font-mono text-xs bg-input px-1.5 py-px rounded border border-border"
                  >
                    {ip.slice(1, -1)}
                  </code>
                );
