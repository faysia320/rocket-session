...
export function FilePanel({ sessionId }) {
  const { fileChanges } = useClaudeSocket(sessionId);
          ...
          fileChanges.map((change, i) => (
            <div key={i} style={styles.item}>
              <div style={styles.itemHeader}>
                <span style={{
                  ...styles.toolBadge,
                  background: change.tool === 'Write' ? 'rgba(34,197,94,0.15)' :
                              change.tool === 'Edit' ? 'rgba(59,130,246,0.15)' :
                              'rgba(148,163,184,0.15)',
                  color: change.tool === 'Write' ? 'var(--success)' :
                         change.tool === 'Edit' ? 'var(--info)' : 'var(--text-muted)',
                }}>
                  {change.tool}
                </span>
