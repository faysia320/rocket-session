...
import { getContextWindowSize, getModelDisplayName } from "@/lib/modelContextMap";
...
export const ContextWindowBar = memo(function ContextWindowBar({
  ...
  const estimate = useMemo(() => {
    ...
    const remainingTurns = Math.max(0, Math.floor(remainingTokens / tokensPerTurn));
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="flex items-center gap-1.5 cursor-default">
          {modelName ? <span className="font-mono text-2xs text-info/70">{modelName}</span> : null}
      ...
      <TooltipContent side="bottom" className="font-mono text-xs">
        <div className="space-y-0.5">
          {modelName ? <div className="text-info font-semibold">{modelName}</div> : null}
          ...
          {estimate ? (
            <div className="pt-0.5 border-t border-border">
              <div>평균 {formatTokens(Math.round(estimate.tokensPerTurn))}/턴</div>
              <div
                className={cn(
                  estimate.remainingTurns <= 2
                    ? "text-destructive"
