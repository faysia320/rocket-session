...
describe('useClaudeSocket.utils', () => {
  ...
  describe('getBackoffDelay', () => {
    it('지수 백오프를 적용한다 (attempt 증가 시 대략 2배)', () => {
      ...
      const avg0 = Array.from({ length: iterations }, () => getBackoffDelay(0))
        .reduce((sum, v) => sum + v, 0) / iterations;
      const avg1 = Array.from({ length: iterations }, () => getBackoffDelay(1))
        .reduce((sum, v) => sum + v, 0) / iterations;
      const avg2 = Array.from({ length: iterations }, () => getBackoffDelay(2))
        .reduce((sum, v) => sum + v, 0) / iterations;
    ...
    it('지터 범위가 delay * 0.8 ~ delay * 1.2 사이다', () => {
      const attempt = 3;
      const expectedBase = Math.min(RECONNECT_BASE_DELAY * Math.pow(2, attempt), RECONNECT_MAX_DELAY);
