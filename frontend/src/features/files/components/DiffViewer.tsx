  ...
  return (
    <div className="font-mono text-[11px] leading-[1.6] select-text">
      {lines.map((line, i) => {
        if (line.type === 'info') {
          return (
            <div key={i} className="px-4 py-0.5 text-muted-foreground/70 bg-secondary/30 italic">
              {line.content}
            </div>
          );
        }
        if (line.type === 'header') {
          return (
            <div key={i} className="px-4 py-1 text-info bg-info/10 border-y border-border/30 font-semibold mt-2 first:mt-0">
              {line.content}
            </div>
          );
        }
